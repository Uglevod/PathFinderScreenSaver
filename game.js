class Game {
    ...
    updateEntityMovement(entity, deltaTime) {
        if (entity.x !== entity.targetX || entity.y !== entity.targetY) {
            ...
            // Добавляем обновление шлейфа для бегунков
            if (entity.type === 'runner' && entity.trail) {
                // Добавляем текущую позицию в шлейф
                if (entity.moveProgress % 0.1 < deltaTime * speed || entity.trail.length === 0) {
                    entity.trail.push({
                        x: entity.displayX,
                        y: entity.displayY,
                        opacity: 1.0,
                        timestamp: performance.now() // Добавляем временную метку
                    });
                ...
                // Обновляем прозрачность точек шлейфа
                const now = performance.now();
                for (let i = 0; i < entity.trail.length; i++) {
                    // Вычисляем прозрачность на основе положения в шлейфе и времени
                    const positionFactor = 1 - (i / entity.trail.length); // 1.0 для самой свежей точки, ближе к 0 для старых
                    const timeFactor = 1 - Math.min(1, (now - entity.trail[i].timestamp) / 5000); // Время жизни точки (5 секунд)
                    
                    // Комбинируем оба фактора и применяем дополнительное затухание
                    entity.trail[i].opacity = positionFactor * timeFactor * (1 - TRAIL_FADE_SPEED * deltaTime);
                }
                
                // Удаляем невидимые точки шлейфа
                entity.trail = entity.trail.filter(point => point.opacity > 0.05);
            }
        }
    }
    ...
    planRunnerMove(runner) {
        ...
        // Проверяем наличие выхода
        for (const exit of this.exits) {
            if (runner.x === exit.x && runner.y === exit.y) {
                // Бегунок достиг выхода, перемещаем его за границы лабиринта
                // ...существующий код...
                return;
            }
        }
        
        // Если бегунок находится в той же позиции несколько раз, сбрасываем кэшированный путь
        // ...существующий код...
        
        // Проверяем, можно ли продолжить движение в текущем направлении
        let canContinueCurrentDirection = false;
        
        if (runner.currentDirection >= 0) {
            const dir = directions[runner.currentDirection];
            const newX = runner.x + dir.dx;
            const newY = runner.y + dir.dy;
            
            // Проверяем, доступно ли продолжение текущего направления
            if (
                newX >= 0 && newX < MAZE_SIZE &&
                newY >= 0 && newY < MAZE_SIZE &&
                this.maze[newY][newX] === 0 && // Не стена
                !this.isPositionOccupied(newX, newY, runner) // Не занято
            ) {
                canContinueCurrentDirection = true;
                validDirections.push(runner.currentDirection);
            } else {
                // Встретили препятствие, сбрасываем текущее направление
                runner.currentDirection = -1;
            }
        }
        
        // Если не можем продолжить в текущем направлении, ищем все доступные направления
        if (!canContinueCurrentDirection) {
        ...
        if (validDirections.length > 0) {
            if (canContinueCurrentDirection) {
                // Если можем продолжить в текущем направлении, всегда делаем это
                dirIndex = runner.currentDirection;
            } else {
                // Иначе выбираем направление на основе стратегии исследования
        ...
        if (dirIndex >= 0) {
            ...
            // Запоминаем новое текущее направление
            runner.currentDirection = dirIndex;
        }
    }
        ...
        if (this.entities.filter(e => e.type === 'runner').length < MAX_RUNNERS) {
            if (Math.random() < 0.01) {
                ...
                const runner = {
                    ...
                    explorationLevel: 0.8 + Math.random() * 0.2,
                    explorationMemory: 20 + Math.floor(Math.random() * 30),
                    currentDirection: -1
                };
