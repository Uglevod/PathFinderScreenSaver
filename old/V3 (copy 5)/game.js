class Game {
    ...
    checkCollisions() {
        // Проверяем бегунков, которые нашли выход
        const runnersToRemove = [];
        const borderSize = 2; // Размер внешнего коридора
        
        for (const entity of this.entities) {
            if (entity.type === 'runner') {
                // Проверяем, завершил ли бегунок движение
                if (entity.moveProgress >= 1.0) {
                    // Считаем бегунка сбежавшим, если он находится в внешнем коридоре
                    if (
                        entity.x < borderSize || 
                        entity.x >= MAZE_SIZE - borderSize || 
                        entity.y < borderSize || 
                        entity.y >= MAZE_SIZE - borderSize
                    ) {
                        // Бегунок за внешней стеной, значит сбежал
                        runnersToRemove.push(entity);
                        this.stats.runnersEscaped++;
                        
                        // Обновляем процент успешных побегов
                        ...
                        console.log(`Бегунок ${entity.id} сбежал на позиции (${entity.x}, ${entity.y})!`);
                    }
                }
            }
        }
        
        // Удаляем сбежавших бегунков
