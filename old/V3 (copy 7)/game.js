class Game {
    ...
    checkCollisions() {
        // Проверяем бегунков, которые оказались за стеной
        const runnersToRemove = [];
        const borderSize = 2; // Размер внешнего коридора
        
        for (const entity of this.entities) {
            if (entity.type === 'runner') {
                // Проверяем текущее положение (не ждем завершения движения)
                // Используем displayX/Y для точного определения позиции
                const x = Math.floor(entity.displayX);
                const y = Math.floor(entity.displayY);
                
                // Если бегунок за пределами лабиринта или во внешнем коридоре - удаляем
                if (
                    x < 0 || x >= MAZE_SIZE || 
                    y < 0 || y >= MAZE_SIZE ||
